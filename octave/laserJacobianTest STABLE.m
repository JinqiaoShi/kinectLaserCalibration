...
for i=1:iterations
	...
	for i=1:size(Z,2)
		alfa=Z(4,i);
		scan=Z(3,i);
		J(1,1)=(alfa^2*x(2)*scan^2+scan^2)*(2*alfa*x(1)*x(2)*scan^2+x(1)*scan^2)  +  scan+scan^2*x(1)+scan^2*alfa^2*x(1)*x(2);
		J(1,2)=(2*alfa^2*x(1)*scan^2)*(2*alfa*x(1)*x(2)*scan^2+x(1)*scan^2)  +  scan+scan^2*x(1)+scan^2*alfa^2*x(1)*x(2);
		K=(Z(1,i)-(scan+scan^2*x(1)+scan^2*alfa^2*x(1)*x(2)))^2;
		H+=J'*J;
		B+=(J'*K);						
	endfor
	dx=-H\B;
...
for i=1:size(laser,1)
	alfa=laser(i,4);
	scan=laser(i,3);
	laser(i,3)=scan+scan^2*x(1)+scan^2*alfa^2*x(1)*x(2);
endfor
